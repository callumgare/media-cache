...
async function queryMediaWhere(condition: QueryGroupCondition) {
  ...
  return db.select({ id: dbSchema.cacheMedia.id, title: dbSchema.cacheMedia.title, finderSourceMediaIds: dbSchema.cacheMedia.finderSourceMediaIds })
    .from(dbSchema.cacheMedia)
    .where(whereClause)
}

function makeCondition(overrides: Partial<QueryGroupCondition> = {}): QueryGroupCondition {
  return {
    id: 1,
    type: 'group',
    operator: 'AND',
...
async function seedMedia() {
  ...
  enqueueMedia([
    ...
    { mediaFinderSource: 'test-source', id: 'vid-c', title: 'Video C (audio)', tags: [], files: [{ type: 'main', url: 'https://example.com/c.mp4', video: true, audio: true, image: false }] },
  ])
...
describe('/api/media — AND conditions', () => {
  ...
  it('AND with contradictory conditions returns empty', async () => {
    ...
    const results = await queryMediaWhere(makeCondition({
      operator: 'AND',
      conditions: [
        { id: 2, type: 'field', field: 'type', operator: 'equals', value: 'video' },
        { id: 3, type: 'field', field: 'type', operator: 'equals', value: 'image' },
      ],
    }))
    expect(results).toHaveLength(0)
  })
...
describe('/api/media — OR conditions', () => {
  it('combines type conditions with OR', async () => {
    ...
    const results = await queryMediaWhere(makeCondition({
      operator: 'OR',
      conditions: [
        { id: 2, type: 'field', field: 'type', operator: 'equals', value: 'image' },
        { id: 3, type: 'field', field: 'type', operator: 'equals', value: 'video-with-audio' },
      ],
    }))
    const titles = results.map(r => r.title).sort()
    expect(titles).toContain('Image A')
    ...
    const results = await queryMediaWhere(makeCondition({
      ...
      conditions: [
        // Image with cats tag
        {
          ...
          operator: 'AND',
          conditions: [
            { id: 11, type: 'field', field: 'type', operator: 'equals', value: 'image' },
            { id: 12, type: 'field', field: 'tags', operator: 'includes all', value: [catsId] },
          ],
        },
        // OR video-with-audio
        { id: 13, type: 'field', field: 'type', operator: 'equals', value: 'video-with-audio' },
      ],
    }))
    const titles = results.map(r => r.title).sort()
    expect(titles).toContain('Image A')
