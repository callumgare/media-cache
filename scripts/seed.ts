...
function buildSingleMedia(
  ...
  const mediaSources = faker.helpers.arrayElements(sourceRecords, {
    min: 1,
    max: 2,
  });
  ...
  const duration = hasVideo
    ? faker.number.float({ min: 10, max: 7200, fractionDigits: 1 })
    : null;
  const width = hasVideo
    ? 1280
    : hasImage
      ? faker.helpers.arrayElement([800, 1200, 1920])
      : null;
  const height = hasVideo
    ? 720
    : hasImage
      ? faker.helpers.arrayElement([600, 900, 1080])
      : null;
...
const sampleVideos = [
  ...
  // VideoLAN (VLC) test streams
  {
    width: 320,
    height: 176,
    url: "https://streams.videolan.org/streams/mp4/Mr_MrsSmith-h264_aac.mp4",
  },
