...
class QueryExecutionTaskSystem extends EventEmitter implements TaskProvider {
  ...
  async getTasks(): Promise<QueryExecutionTask[]> {
    ...
    for (const exec of dbExecs) {
      ...
      tasks.push(
        runningState ? this.stateToTask(runningState) : this.dbRowToTask(exec),
      );
    }
    ...
    for (const state of this.runningStates.values()) {
      if (!coveredExecutionIds.has(state.executionId)) {
        console.warn(
          `Running execution ${state.executionId} has no corresponding DB entry`,
        );
      }
    }
  ...
  private dbRowToTask(
    exec: dbSchema.FinderQueryExecution & {
      logs?: dbSchema.FinderQueryExecutionLog[];
    },
  ): QueryExecutionTask {
    return {
      type: "query_execution",
