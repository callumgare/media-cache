...
import {
  createCacheMedia, createExecutionLogEntry, createFinderQueryExecution, createFinderQueryMedia,
  createFinderQueryMediaContent, getCacheMedia, updateCacheMedia,
...
export async function runMediaFinderQuery({
  ...
  try {
    ...
    queryExecutionTaskSystem.update(executionId, { status: 'Beginning processing...' })
    const previousExecution = dbFinderQuery
      ? await getPreviousFinderQueryExecution({ queryId: dbFinderQuery.id, currentExecutionId: executionId })
      : null
    ...
    if (affectedCacheMediaMap.size) {
      queryExecutionTaskSystem.update(executionId, { status: 'Updating existing media...' })
    }
    ...
    if (added.length) {
      queryExecutionTaskSystem.update(executionId, { status: 'Adding new media...' })
    }
    ...
    if (dbFinderQuery) {
      await deleteOldFinderQueryMedia({ queryId: dbFinderQuery.id, currentExecutionId: executionId })
    }
    ...
    await createExecutionLogEntry({ executionId, queryId, level: 'fatal_error', message })
