...
Media Cache is a personal media aggregation and browsing app. It pulls media (images and videos) from external sources via the [Liase](https://github.com/callumgare/liase) library, stores the metadata and file references in a local database, and serves them through a unified browsing interface with filtering and full-screen viewing.
...
Stored Liase queries are periodically run and the results are used to created cached media entries. These entries can been be browsed by the user via the web ui.

### 1. Defining Queries

An admin creates a **liase query** via the UI (`/admin/queries`). A query specifies:
...
Queries are serialized and stored in the `liase_query` table.

### 2. Running a Query

A stored query can then be run which first creates a new entry in `liase_query_execution` which includes details about the run itself.

Then the query is executed by liase which returns a bunch of results. An entry for each each result is stored in `liase_query_media` with a hash of the raw JSON content rather than the content itself. This JSON content is instead stored in `liase_query_media_content` with it's content hash so that so that identical results from multiple runs aren't duplicated.

Next the entries in `liase_query_media` from the last run of the saved query (if there was a previous run) is compared against to see which media have been added, updated or removed since last run.

For every media that has changed (added, updated or removed) we need to create/update a `cache_media`. To do this a list of source/media id pairs for all changed media is compiled. A cache_media entry can be be made up from liase media results which can potentially exist in multiple different sources. For each source/media id pair of the changed liase media we look for an existing `cache_media` record. If a record exists and that has other source/media id pairs in it we add that to the list.

Then for each source/media pair we fetch all saved liase results records. Any source/media id pairs that have no saved liase results are considered deleted. For these we check to see if there's an existing `cached_media` record. If there is one and every source/media id pair for it is to be delete then we delete the cached media entry and add to `deleted_cache_media` (if there is a cached media entry but it has some source/media id pairs in it that aren't being deleted we can leave it for now since those pairs will have been added to the update list).

For source/media id pairs that do have saved liase results we merge all matching results into one results media per source/media id pair. These are then used to update or create cached media. If updating then we make sure to check if that existing cached media is made up of liase media from multiple sources. When updating or creating cached media root property values are generally aggregated (`title` is the first truthy title, `views` is the sum of view from all sources, `earliestUploadedAt` is the oldest uploadedAt value in the sources) where as each entry in the sources array will have the exact values from the merged liase results media. All the unique tags find in the sources are put into a list and then a `group` entry for them is created under a root `tags` entry if it doesn't already exist. The id's for these groups are then used for the `groupIds` field in the cache media entry.

Finally records in `liase_query_media` from any previous runs of that liase query are deleted and the `liase_query_execution` is updated.
...
2. If the URL has expired, re-runs the original liase query to refresh it and updates the database.
...
| `source` | Known media sources by their `liase_source_id` string |
| `liase_query` | Saved query definitions |
| `liase_query_execution` | Execution records with stats (found/new/updated/etc.) |
| `liase_query_media` | Links an execution to a piece of media via content hash |
| `liase_query_media_content` | Raw serialized media JSON, deduplicated by content hash |
| `cache_media_file_update` | History of file URL refreshes |

## Key Columns on `cache_media`

- `liase_source_media_ids` — `text[][]` — array of `[sourceId, mediaId]` pairs; a single media item can be found via multiple sources
...
The `@liase/core` library abstracts external sources behind a plugin interface. Plugins are loaded at startup from the `LIASE_PLUGINS` environment variable (comma-separated file paths). Each plugin declares its available handlers and their parameter schemas (Zod), which the admin query form renders dynamically.
...
| `LIASE_PLUGINS` | Comma-separated paths to Liase plugin files |
