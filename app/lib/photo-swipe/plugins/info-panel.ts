...
function pswpSvg(pathD: string, id: string): string {
  return `<svg aria-hidden="true" class="pswp__icn" viewBox="0 0 32 32" width="32" height="32">`
    + `<use class="pswp__icn-shadow" xlink:href="#${id}"/>`
    + `<path d="${pathD}" id="${id}"/>`
    + `</svg>`
}
...
export class PhotoSwipeInfoPanelPlugin {
  ...
  private initPswp(pswp: PhotoSwipe) {
    const { isOpen, currentMedia, panelEl, toggle, _registerSetOpen } = useInfoPanel()
    ...
    pswp.options.paddingFn = () => ({ top: 0, bottom: panelBottomPadding, left: 0, right: 0 })
    ...
    function animatePanelResize(targetPadding: number, fromPanY?: number) {
      ...
      if (fromPanY !== undefined) {
        ...
        if (cs) {
          ...
          toPanY = Math.min(cs.bounds.min.y, Math.max(cs.bounds.max.y, desired))
      ...
      function frame(now: number) {
        ...
        panelBottomPadding = Math.round(startPadding + (targetPadding - startPadding) * ease)
        pswp.updateSize(true)
    ...
    function setOpen(open: boolean, immediate = false) {
      ...
      if (immediate) {
        ...
        requestAnimationFrame(() => pswp.template?.classList.remove('pswp--info-no-transition'))
      }
    ...
    pswp.on('pointerUp', (e) => {
      if (e.originalEvent.pointerType !== 'mouse') numActivePointers = Math.max(0, numActivePointers - 1)
      ...
      if (upwardDragActive) {
        ...
        const recentVP = upVelocityPoints.filter(p => performance.now() - p.t < 150)
        let velocityUp = 0
        ...
        if (vpFirst && vpLast && recentVP.length >= 2) {
          ...
          velocityUp = (vpFirst.panY - vpLast.panY) / Math.max(1, vpLast.t - vpFirst.t)
        }
        upVelocityPoints = []
      ...
      if (downwardPanelDragActive) {
        ...
        const recentDown = downVelocityPoints.filter(p => performance.now() - p.t < 150)
        let velocityDown = 0
        ...
        if (dvFirst && dvLast && recentDown.length >= 2) {
          ...
          velocityDown = (dvLast.panY - dvFirst.panY) / Math.max(1, dvLast.t - dvFirst.t)
        }
        downVelocityPoints = []
      ...
      if (!upwardDragActive && !isOpen.value && numActivePointers === 0 && ptrDownAtMinBound && e.originalEvent.pointerType !== 'mouse') {
        ...
        if (flickVelocity > 0.3 && dy > 5) {
          ...
          pswp.applyBgOpacity(1)
          animatePanelResize(Math.round(pswp.viewportSize.y * 0.5), draggedPanY)
        }
      }
    })
    ...
    pswp.on('uiRegister', () => {
      pswp.ui?.registerElement({
        ...
        html: {
          isCustomSVG: true,
          inner: '<path d="M8 16a8 8 0 1 0 16 0 8 8 0 0 0-16 0z" id="pswp__icn-info"/><path fill="currentColor" d="M15 11.5h2v2h-2z"/><path fill="currentColor" d="M15 15h2v5h-2z"/>',
    ...
    pswp.on('appendHeavy', () => {
      if (!panelEl.value && pswp.template) {
        ...
        el.addEventListener('pointerup', (e: PointerEvent) => {
          ...
          if (firstPoint && lastPoint && recent.length >= 2) {
            velocity = (lastPoint.y - firstPoint.y) / Math.max(1, lastPoint.t - firstPoint.t)
          }
          ...
          if (dy > 60 || velocity > 0.5) {
            ...
            el.style.transition = 'transform 0.3s ease'
            el.style.transform = ''
            animateSlidePan(pswp.currSlide?.pan.y ?? panelDragStartSlideY, panelDragStartSlideY)
            setTimeout(() => {
              el.style.transition = ''
            }, 300)
          }
      ...
      currentMedia.value = (pswp.currSlide as SlideWithMedia | undefined)?.data?.mediaData ?? null
    })
