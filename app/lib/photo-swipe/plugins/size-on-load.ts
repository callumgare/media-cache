...
export class PhotoSwipeSizeOnLoadPlugin {
  ...
  initLightboxEvents(_lightbox: PhotoSwipeLightbox) {
  ...
  updateLoadedSlideSizesIfNeeded(pswp: PhotoSwipe) {
    ...
    const updateNeeded = slidesInDom.some(
      (slide) =>
  ...
  detectSlideMediaSize({
    content,
    mediaElement,
    pswp,
  }: { content: Content; mediaElement: HTMLElement; pswp: PhotoSwipe }) {
    if (
      content.data.width ||
      content.data.height ||
      !mediaElement ||
      this.mediaCurrentlyLoading.get(mediaElement)
    )
      return;
  ...
  updateSlideSizeFromMedia({
    content,
    pswp,
  }: { content: Content; pswp: PhotoSwipe }) {
    ...
    let mediaSize: ReturnType<typeof mediaElementSize> | undefined;
  ...
  detectSlideSize({ content, pswp }: { content: Content; pswp: PhotoSwipe }) {
    ...
    if (content.element) {
      if (!sizeKnown) {
        this.detectSlideMediaSize({
          content,
          mediaElement: content.element,
          pswp,
        });
      }
    }
    if (content.placeholder?.element) {
      if (!sizeKnown) {
        this.detectSlideMediaSize({
          content,
          mediaElement: content.placeholder.element,
          pswp,
        });
      }
    }
